...
**Critério de conclusão:** `npm run build` passa sem erros, banco sobe com `docker compose up`, migration roda com `prisma migrate dev`.
...
**Papéis sugeridos dentro do Clerk (Roles):**
...
**Critério de conclusão:** tesoureiro consegue cadastrar, editar e buscar irmãos. `notes_internal` não aparece em nenhum endpoint acessível pelo papel `irmao`.
...
- Criar tabela:
  ```
  charge_types (id, name, description, default_amount, is_recurring, active, created_at, updated_at)
  ```
...
- Criar tabela:
  ```
  charges (id, member_id, charge_type_id, competence_date, due_date, description,
...
- Criar tabelas:
  ```
  payments (id, member_id, payment_date, amount, payment_method, notes,
            created_by, created_at, updated_at)

  payment_allocations (id, payment_id, charge_id, allocated_amount)
  ```
- Registrar pagamento: data, valor, forma, observação, comprovante (opcional via `attachments`)
- Criar tabela de anexos:
  ```
  attachments (id, owner_type, owner_id, file_url, file_name, uploaded_by, created_at)
  ```
...
- Suporte a pagamento excedente: se o valor pago superar as cobranças, registrar `credit_balance` no membro
  - Adicionar campo `credit_balance DECIMAL(10,2) DEFAULT 0` na tabela `members`
- Estorno: reverter alocação e atualizar status da cobrança de volta para `estornada`
...
- Nunca excluir `payment_allocations` — apenas estornar
- O saldo do irmão = `credit_balance` (créditos não alocados)
...
- Criar serviço backend dedicado: `modules/reports/pdf.service.ts`
...
- Endpoint de download: `GET /api/members/[id]/extrato.pdf`
...
- Criar tabela:
  ```
  recurring_charge_rules (id, charge_type_id, frequency, amount, active, params_json,
...
- Criar tabela:
  ```
  cash_transactions (id, type, category, description, amount, transaction_date,
...
- Criar tabela:
  ```
  period_closings (id, competence_month, competence_year, closed_by, closed_at,
                   total_charged, total_received, balance, notes)
  ```
...
- Multi-loja nativo via **Clerk Organizations**: a arquitetura da base atual do *Dashboard Starter* é construída ao redor do Clerk Organizations, que provê isolamento Multi-Tenant imediatamente.
  - Exigirá apenas assegurar no backend que eventuais buscas no Prisma contenham a restrição do `org_id` recuperado do contexto autenticado da API.

**Critério de conclusão:** API documentada (Swagger ou similar). Schema preparado para `lodge_id` sem quebrar o funcionamento atual.
...
| Saldo credor: campo vs entrada em tabela | Parte 1.6 | `credit_balance` em `members` ou entrada especial em `payment_allocations` | Campo `credit_balance` em `members` — queries mais simples |
...
*Documento gerado com base no PRD da Loja Maçônica — revisão incorporando sugestões de arquitetura.*
